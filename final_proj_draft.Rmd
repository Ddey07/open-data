...
In general, there are different types of discrete regression models that people have used so far to analyze goals and shots in soccer.[@goddard05] used bivariate Poisson and ordered probit regression models to forecast number of goals scored and conceded in soccer matches based on different covariates. The authors here analyzed data from English football and learned that the number of goals in a match depends mostly on the recent home performances of the home side and the recent away performances of the away side. Player-related or shot-related approaches have been studied by a few authors as well. One of the most popular methods in this regard is a study in[@asa16]. Analyzing data from the Major League Soccer (MLS), the authors showed that the shot conversion rates significantly depend on many factors, including the distance, angle and the body part used when a shot is taken. Simple logistic regression techniques were the main components of this study. In an earlier paper,[@moura07] dealt with data from Brazilian first division championship matches and analyzed shots to goal strategies based on the field position from where the ball was collected and the length of the passing sequence leading to a goal. Contrary to the popular belief, they found that the ball possessions resulting in shots to goal usually start in the defensive field of the scoring teams. So far as the passing sequences are concerned, this study showed that either short (2 to 5) or very long (10 or more) sequences significantly improve the conversion rates. This second finding corroborated what[@hughes05] found before. 

Finally, we cite two works that used the situation in a small time-window just preceding the event of interest. First, [@jordet13] used English Premier League data to investigate how visual exploratory behaviors before receiving the ball affect the probability of completing the next pass. They found that the players who show extensive visual exploratory behaviors right before receiving a ball are more successful in completing a pass. In another recent study,[@lucey14] analyzed the spatiotemporal patterns of the ten-second window of play before a shot was fired. They extracted some strategic features from the data and used them to present a method that estimates the likelihood of a team scoring a goal. This study showed that not only is the game phase (corner, free-kick, open-play, counter attack etc.) important, but the features such as defender proximity, interaction of surrounding players and speed of play coupled with the shot location also play an impact on determining the likelihood of a team scoring a goal. 

We aim to unify these approaches to make insightful graphics helpful for managers to select their teams and provide real-time game control prediction to improve during game management process. 
...
The data was downloaded [here](https://github.com/statsbomb/open-data?fbclid=IwAR1Ddi9zAatINELOuo8BCTsx0sRSD5u33R27p1IwQrTsC_pQOMbTRhG3BKo) in JSON format and then processed with the R code below. 
...
where $Z(x_i,y_i)$ is the estimated expected goals surface for turnover locations. We take absolute value of the $y$ axis co-ordinates as we expect the inherent pattern to be symmetric with respect to $x$-axis on the soccer field. We perform tests of complete spatial randomness for a given point pattern, based on quadrat counts and found significant evidence against null hypothesis (p-value $<2.2e-16$).
...
quadrat.test(PP.shot,nx=3,ny=3)
...
DF.shot.heatmap <- function(DF){
  ...
  G2=ggplot(pred.df,aes(x=x,y=y)) +
    ...
    labs(fill="Intensity", 
         title=paste("Intensity of locations of shots conceded from open play"))
  
  L <- length(teamnames)
  teamnames1 <- teamnames[1:as.integer(L/2)]
  teamnames2 <- teamnames[-(1:as.integer(L/2))]
  ...
  grid.arrange(G1, G2, nrow = 2, top=paste("Team Formation:",DF), 
    ...
    grid.arrange(G1, G2, nrow = 2, top=paste("Team Formation:",DF), 
               bottom=paste("Teams:", paste(teamnames1,collapse = ", "),"\n",paste(teamnames2,collapse = ", ")))
  }
...
##Cross-validated AUC
train$Y <- factor(train$shot!=0)
levels(train$Y) <- c("A","B")
fit.glm <-glm(Y ~ timespan + pass.length + pass.speed + I(time.under.pressure/timespan) + I(pass.under.pressure/passcount) + passcount + chull.area + dist.to.goal + vert.dist, data=train, family=binomial())
fit.glm <- step(fit.glm)

control <- trainControl(method="cv", number=10, classProbs = TRUE,summaryFunction = twoClassSummary)
metric <- "ROC"

fit_glm <- train(formula(fit.glm), data=train,
                     method="glm", metric=metric, trControl=control,tuneLength = 30)
...
fit_xg <- train(formula(fit.xg), data=subset(train,shot != 0),
                     method="glm", family=Gamma(),metric=metric, trControl=control,tuneLength = 30)
...
### Test RMSE
sqrt(mean(((test$xG) - fit.xg.pred)^2))
...
We use backward selection to get optimum set of predictors and observe a 10-fold cross-validated AUC for $0.95$ for shot prediction on training set and AUC of $0.95$ on test. Also, we observe 10-fold cross-validated root mean-squared error of $0.08$ for xG on train set and RMSE for test set to be $0.04$.
...
We began with the thought that two things will be very important for success in a soccer game: 1) controlling possession of the ball, and 2) putting yourself in the best position to score.  Because scoring a goal is such a low percentage event, by maximizing those two elements a team becomes their most competetive.  

In the heatmaps we created to address [Question 1], we find that the shot locations don't differ much according to formations except $3-5-2$, where we see players have to get to be relatively closer to the goal before taking shot, meaning the defenders cover out the space more around final third of the pitch, leaving less room for shots. More interestingly, the vulnerability zone, (the area of the pitch where turning over the ball can be costly by leading to a shot) looks different for different defending team formations. This validates common beliefs like formations like $3-4-3$ or $3-5-2$ don't cover enough spaces on the wings whereas formations like $4-4-2$ can leave space in the middle. Investigating further into this, an inquisitive mind would certainly ask if the goalkeepers are sending the passes to correct region of vulnerability depending on the opposition formation. We can find the answer from the goalkeeper passes intensity heatmap and it looks like they sometimes conform to it but not always. Hence, through our visualizations, we can devise defence straegies like where to put your best defender according to your formation and also attacking instructions like where to hit the goal-kick or send the long passes based on your opponent's formation. 

Due to limited time and resources, we can't address team-specific vulnerability zones but following the same approach, we can help managers pick starting XI and devise strategies based on a team's vulnerability zone heatmap based on the past games of the season. 

Now coming to our next question, our end goal was to build a dashboard which will help track a team's game control in real time and thus help the manager to make necessary changes during game. To do that, we perform a possession-wise analysis and score each possession string based on its potential to turn into shots or goals. Based on the quality of the possession a team had and the possessions the team conceded, we can update a team's expected game control over time. This gives the managers time to react during game and make necessary changes to influence the result. 

Extending this idea, we can score each player based on the score of the possessions the player was involved in. Due to limited time, we couldn't implement this idea but its just a one step up where we can quantify a player's in-game involvement both from the attacking and defensive side. This might be crucial in signing players where you want to replace a certain player with a similar one but who costs you less amount of money. We believe, one the one hand the real-time updating of team's control scores will help game-changing decisions and on the other hand, the player scores will help immensely in scouting new talents. 
